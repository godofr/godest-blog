...
def restaurar_bloques_codigo(org_text, placeholders_codigo):
    ...
    for key, contenido in placeholders_codigo.items():
        ...
        bloque_limpio = f"#+begin_example\n{contenido}\n#+end_example"

        # Caso A: pandoc generó #+BEGIN_EXAMPLE envolviendo nuestro placeholder
        # en una sola línea → eliminar wrapper y dejar solo nuestro bloque
        patron_wrapper = r'#\+BEGIN_EXAMPLE[^\n]*\n\s*' + re.escape(key) + r'\s*\n#\+END_EXAMPLE'
        if re.search(patron_wrapper, org_text, flags=re.DOTALL | re.IGNORECASE):
            org_text = re.sub(
                patron_wrapper,
                lambda m, b=bloque_limpio: b,
                org_text,
                flags=re.DOTALL | re.IGNORECASE,
            )
        # Caso B: placeholder dentro de un bloque example multilínea
        elif re.search(r'#\+[Bb][Ee][Gg][Ii][Nn]_[Ee][Xx][Aa][Mm][Pp][Ll][Ee].*?' + re.escape(key),
                       org_text, flags=re.DOTALL):
            patron = r'#\+[Bb][Ee][Gg][Ii][Nn]_[Ee][Xx][Aa][Mm][Pp][Ll][Ee]\s*\n.*?' + re.escape(key) + r'.*?\n#\+[Ee][Nn][Dd]_[Ee][Xx][Aa][Mm][Pp][Ll][Ee]'
            org_text = re.sub(
                patron,
                lambda m, b=bloque_limpio: b,
                org_text,
            ...
            # Caso C: placeholder suelto
            org_text = org_text.replace(key, f"\n{bloque_limpio}\n")
    return org_text
...
def restaurar_imagenes(org_text, placeholders):
    ...
    dentro_de_export = False
    for linea in lineas:
        # Rastrear si estamos dentro de un bloque export (no resetear lista)
        if linea.strip().startswith('#+begin_export'):
            dentro_de_export = True
        elif linea.strip().startswith('#+end_export'):
            dentro_de_export = False
        ...
        # - activar si encontramos item numerado
        # - NO desactivar si estamos dentro de export o línea en blanco
        if patron_item_lista.match(linea):
            ultimo_item_lista = True
        elif linea.strip() and not dentro_de_export and not linea.strip().startswith('#+'):
            ultimo_item_lista = False
