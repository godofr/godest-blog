...
def extraer_bloques_codigo(html):
    """
    FIX #1: Extrae bloques <pre><code> antes de pandoc para preservar
...
def restaurar_bloques_codigo(org_text, placeholders_codigo):
    ...
    for key, contenido in placeholders_codigo.items():
        ...
        bloque = f"\n#+begin_example\n{contenido}\n#+end_example\n"
        # El placeholder puede aparecer dentro de un #+begin_example generado por pandoc
        org_text = re.sub(
            r'#\+begin[_\s]example\s*\n.*?' + re.escape(key) + r'.*?\n#\+end[_\s]example',
            bloque.strip(),
            org_text,
            flags=re.DOTALL | re.IGNORECASE,
        )
        # O puede aparecer suelto si pandoc lo dejó en otra forma
        org_text = org_text.replace(key, bloque)
    return org_text


def normalizar_saltos_linea(html):
    """Convierte \\n simples entre texto en <br>."""
    html = html.replace('\r\n', '\n').replace('\r', '\n')
    html = re.sub(
        r'(?<!\n)\n(?!\n)(?!<(?:/?(?:p|h[1-6]|ul|ol|li|blockquote|pre|div|table|tr|td|th|hr)))',
        '<br>\n', html
    )
    return html
